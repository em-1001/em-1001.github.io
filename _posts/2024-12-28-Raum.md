...
1. **Native (T1)**: T1 weighted image는 T2 weighted image보다 신호강도가 높아 해부학적 구조물을 좀 더 명확하게 구별 가능하다. T1영상에서 물은 어둡게 보이며, 낭종은 저신호강도로 검게, 피하지방조직은 고신호강도로 희게, 근육은 중신호강도를 보인다.       
...
3. **T2-weighted (T2):** T2 weighted image는 물(e.g. CSF)이 매우 하얗게 보인다. 수분 함유량에 대체로 비례하여 낭종이 가장 하얗게 보이고, 이어서 부종이 동반된 조직, 정상 조직 순으로 보인다.   
4. **T2-FLAIR (T2 - Fluid Attenuated Inversion Recovery):** FLAIR는 T2에 정상적인 CSF를 어둡게 나타낸다. 문제가 있는 CSF는 밝게 남아있으며 정상 CSF와 abnormality를 정말 쉽게 구별해낼 수 있다. 
