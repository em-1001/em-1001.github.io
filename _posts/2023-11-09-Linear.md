...
실제로 공분산 행렬이 설명하는 데이터는 x 데이터의 분산에 비해 어디에 위치하는지, y 데이터의 분산에 비해 어디에 위치하는지, 그리고 공분산에 비해 어디에 위치하는지를 설명한다. 데이터는 결국 공분산 행렬로 설명되고, Eigen Vector는 행렬이 벡터에 영향을 미치는 주축 방향이므로 공분산 행렬의 Eigen Vector는 데이터가 어떤 방향으로 가장 크게 분산되어 있는지를 알려준다. 그래서 공분산 행렬의 Dominant Eigen Vector 방향으로 데이터가 가장 길게 늘어서고, Eigen Value만큼의 크기를 갖는다. 따라서 Dominant Eigen Vector에 데이터를 사영한다면 잃어버리는 정보를 최소화 할 수 있다. 이렇게 공분산의 Dominant Eigen Vector가 바로 분산이 제일 큰 방향을 가리키는 것을 Principal Component라 한다. 

<p align="center"><img src="https://github.com/user-attachments/assets/d9de7f10-0fce-45f4-9abc-31e1d259f30d" height="" width=""></p>

예를 들어 좌표계에서 (2, 3)이라는 데이터는 $2 \cdot (S_X^2 + S_{XY}) + 3 \cdot (S_Y^2 + S_{YX})$ 이므로 평균으로부터 X 분산의 2배, 평균으로부터 Y 분산의 3배, 평균으로부터 XY 공분산의 5배 떨어진 곳의 점이라 생각할 수 있다. 

2차원을 1차원으로 줄이는 것 뿐만 아니라 n개 차원도 똑같이 적용된다. n개 차원은 
