...
Gram-Schmidt Orthogonalization에서 $\mu_ {i,j}$ 대신 $\lfloor \mu_ {i,j} \rceil b_ j$를 사용하여 Size Reduced 조건을 만족시킨다. Gram-Schmidt가 끝난 뒤에는 모든 $i$에 대해 Lovász condition을 확인하여 조건을 만족하지 않으면 swap한다. 
