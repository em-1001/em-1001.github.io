...
matrix $T_{i,j}$를 basis의 왼쪽에 행렬곱하면 $i$번째 basis와 $j$번째 basis를 swap할 수 있다(vector-switching). matrix $L_{i,j}(k)$를 basis의 왼쪽에 행렬곱하면 $j$번째 basis에 $i$번째 basis를 k번 더한 것과 같다(vector-addition). 중요한 점은 이 두 행렬 모두 determinants가 $\pm 1$이라는 것이다. 즉 이 두 행렬을 이용해 basis에 transformation을 해도 같은 lattice를 represent한다. 이 두 transformation은 LLL algorithm에서 중요한 역할을 한다. 
