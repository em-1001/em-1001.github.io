...
&ensp;&emsp;&emsp;**if** $\exists i$ such that $\lVert b_{i+1}^* + \mu_{i+1, i}b_i^* \rVert^2 \geq \delta\lVert b_i^* \rVert^2$ **then** Swap $b_i$ and $b_{i+1}$ &ensp;(Lovász condition)    
&ensp;&emsp;&emsp;**else**  
&ensp;&emsp;&emsp;&emsp;**return** $\lbrace b_1, \cdots, b_n\rbrace$

Gram-Schmidt Orthogonalization에서 $\mu_ {i,j}$ 대신 $\lfloor \mu_ {i,j} \rceil$를 사용하여 Size Reduced 조건을 만족시킨다. Gram-Schmidt가 끝난 뒤에는 모든 $i$에 대해 Lovász condition을 확인하여 조건을 만족하지 않으면 swap한다. 
...
&= \vert \sum_{i=1}^n x_i \langle b_i, b_j^* \rangle \vert &emsp; \text{by linearity of the inner product} \\ 
&= \vert x_j \vert \langle b_j, b_j^* \rangle &emsp; \text{since} b_i \text{and} b_j^* \text{are orthogonal for} j<i \text{and} x_i=0 \text{for} i<j \\ 
