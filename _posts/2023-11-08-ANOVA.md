...
ANOVA의 Alternative Hypothesis는 최소한 1개의 집단은 평균이 다르다이므로 어느 것이 다른지를 찾아야 한다. 앞서 말했듯이 t test를 $_{n}C_{2}$번 해서 찾아내는 것은 $1 - (1 - \alpha)^{_{n}C _{2}}$ 로 유의수준이 커지므로 Type I Error가 늘어난다. 
