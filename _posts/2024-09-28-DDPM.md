...
$$p(z|x)$$ 가 무엇인지 알지 못하기 때문에, 이미 알고 있는 확률 분포(가우시안..) $$q_{\phi}(z|x)$$ 를 임의로 택하고 그것의 파라미터 $$\phi$$ 를 조정하여 $$p(z|x)$$ 와 유사하게 되도록 하는 것이다. 그렇게 이상적인 확률분포에 근사된 $$q_{\phi}$$ 를 통해서 $$z$$ 를 sampling하게 된다. 
