...
이전에 독립변수를 추가하면 $R^2$는 최소한 같거나 커진다고 한 적이 있다. T = R + E 인데, 여기서 T는 고정이다. 따라서 R이 커지면, E가 줄고, R이 줄어들면, E가 커진다. SST는 관측과 엉망회귀의 차이이므로 독립변수가 추가되었다고 해서 바뀔일이 없다. $R^2 = \frac{SSR}{SST} = \frac{SST - SSE}{SST} = 1 - \frac{SSE}{SST}$ 이고, 독립변수가 추가되면, $R^2$이 변할 텐데, SST는 그대로인데 SSR은 엉망회귀선으로부터 회귀 추세선까지의 차이이므로 R의 변화가 0보다 크거나 같은 값이 된다. 
