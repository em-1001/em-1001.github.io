...
다중공선성을 해결하기 위해서 첫 번째로 만약 계수가 통계적으로 유의하다면, VIF가 크더라도 대처할 필요강 없다. 이는 VIF가 크더라도 계수의 분산이 충분히 작기 때문이다. 두 번째로 계수가 통계적으로 유의하지 않고 VIF가 클 때, 분석에 꼭 필요한 Feature라면 살리는 것이 좋다.  나머지 유의하지 않은 계수중에서 종속변수 y와 나머지 변수의 상관이 적은 것(또는 표준화 계수가 작은 것)과 다중공선성(VIF)이 큰 변수들을 고려하여 주요하게 다중공선성에 영향을 미치는 변수를 제거하는 방법을 고려할 수 있다. 하지만 회귀모형에 고려된 모든 변수를 유의하게 만들기 위해 변수를 마구 제거하는 것은 자료의 다양성을 해치고, 분석하려던 가설이나 이론에 영향을 미칠 수 있기 때문에 가급적이면 하지 않도록 해야 한다. 
제거 대상을 정리하면 다음과 같다. 

1. y종속변수와 상관관계가 작은 독립변수 또는 표준화 상관계수가 작은 독립 변수
2. VIF가 큰 독립변수
3. 측정이 객관적이지 않거나, 불명확해 보이는 독립변수

참고로 해당 관심 변수와 상관이 큰 변수 가 있다고 했을 때, 상관이 크다고 해서 다중공선성을 띈다고는 할 수 없다.  즉 어떤 두 변수가 상관이 높다고 해서 꼭 VIF가 높다고 할 수 없다는 뜻이다. 이런 상관이 큰 변수는 맥락을 잘 보았을 때 매개변수일 가능성이 크다. 

변수를 선택할 때 상관관계가 크면서 VIF가 큰 변수가 매개변수이거나, 통제변수일 경우에는 그냥 두는 것이 낫다. 매개변수의 의미는 인과관계의 논리적인 흐름을 보았을 때의 중간에 끼어 있는 변수를 의미하는데, 특정 독립변수로부터 매개하여 우회적으로 종속변수에 영향을 끼치는 변수이다. 통제변수는 종속변수에 영향을 끼칠 만한 것을 변수로 하여 관심독립변수가 종속변수에게 끼치는 영향을 통제해 종속변수에 대한 영향력을 더 크게 해석하지 않도록 해 주는 변수를 의미이다. 이 변수들은 관심변수만 유의하다면 관심변수의 영향력을 명확하게 설명할 수 있게 해주는 변수들이기 때문에 VIF가 크거나 유의하지 않더라도 그냥 두는 편이 올바른 해석이 될 가능성이 크다. 또한 이런 변수를 이용해서는 추가적인 해석으로 매개효과, 조절효과등을 분석할 수 있게 된다. 

유의하지 않은 계수들을 유의하게 만드는 방법을 정리하면 다음과 같다. 

1. 변수들을 합하거나 차이를 구해서 새로운 변수를 만드는데, 독립변수끼리 서로 상관이 높은 경우에 더하거나 차이를 만들면 변수가 하나로 줄어서 개선될 수 도 있다.

2. PCA를 이용해서 하나의 새로운 변수로 압축할 수도 있다. 특히 PCA를 이용하게 되면 PCA로 압축한 Feature끼리는 직교하게 되어 그 안에서는 다중공선성이 사라지고 이를 Feature Extraction이라 한다.

마지막으로 모든 조건을 만족하는 변수의 조합을 찾아내는 방법이 있다. 설명력이 높은 변수로만 회귀식에 포함하는 방법으로 아무 변수도 추가하지 않은 상태에서 p value가 작은 유의한 변수를 하나하나 추가하면서 가장 좋은 변수의 조합을 찾아내는 방법(전진 선택법)이 있고, 또 하나는 다 넣은 후 p value가 큰 유의하지 않은 변수를 하나하나 제거하면서 적당한 수준의 변수의 조합을 찾아내는 방법이 있다. 

이제 실제 예를 들어 다중공선성을 해결해 볼 것이다. 이때 모든 칼럼에 대해서 각각 정규화를 한 후에 표준화 계수를 구하여 분석하는 것이 분석결과를 해석하는 데에 있어 훨씬 나은 인사이트를 얻을 수 있다. 

이번 예시의 목적은 불필요한 데이터를 제거하여 모델에 남은 모든 계수를 유의하게 만들도록 Feature를 골라내는 것인데, 이를 Feature Selection이라 한다. 아래 예시의 경우는 Feature가 많지 않기 때문에 Feature 압축 (PCA), Feature 더하기 등의 Feature Extraction은 고려하지 않는다. 
