...
$A = \{a_1, ⋯, a_n\}$이라 하면 이 문제를 다음과 같은 방정식으로 변형할 수 있다. 

$$a_1x_1 + ⋯ + a_nx_n - M = 0, x_i = 1 \vert 0$$

이제 $S = \{a_i \vert x_i = 1\}$가 Subset Sum Problem의 해가 되고, $x_1, \cdots, x_n$은 선형이고 작은 해 이므로 LLL을 통해 해결할 수 있다. 
...
아니면 앞서 살펴본 다항식의 계수 문제처럼 $x$를 256이라 생각하여 격자를 아래와 같이 구성할 수도 있다.  
...
다음은 이와 관련된 문제인 SeeTF 2023 Onelinecrypto Writeup으로 Bytes Recovery의 예를 보여준다. 
